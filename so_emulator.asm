...
; ,_     _    / --------------------------------------------
; |\\_,-~/   | This is a nice chunk of assembly code, MEOW! |
...
get_loc:
    ...
    mov     r11b, dil
    and     r11b, 1 
    cmp     r11b, 0                         ; jesli arg jest nieparzyste
    je      .x                              ; jest postaci [X + cos]
    inc     rax                             ; wpp jest postaci [Y + cos]
.x:
    movzx   rax, BYTE [rax]
    cmp     dil, 6                          ; jesli arg <= 6
    jl      .data                           ; nie wystepuje w nim D
    add     al, BYTE [core + 1]             ; wpp jest postaci [cos + D]
.data: 
    add     rax, data
    ret
.register:
...
function:
    ...
    push    rax
    and     rdi, 0x7
    call    get_loc                         ; rax := "wskaznik" na miejsce w pamiecie arg2
    mov     r10, rax                        ; r10 := "wskaznik" na arg1
    pop     rax
    shr     command, 3
    
    cmp     command, 8                      ; MOVI := 0x4000 + 0x100 * arg1 + imm8 -- 8*8 = 0x40
    je      _mov
    cmp     command, 11                     ; XORI := 0x5800 + 0x100 * arg1 + imm8 -- 11*8 = 0x58
    je      _xori
    cmp     command, 12                     ; ADDI := 0x6000 + 0x100 * arg1 + imm8 -- 12*8 = 0x60
    je      _add
    cmp     command, 13                     ; CMPI := 0x6800 + 0x100 * arg1 + imm8 -- 13*8 = 0x68
    je      _cmpi
    cmp     command, 14                     ; RCR := 0x7001 + 0x100 * arg1         -- 14*8 = 0x70
    je      _rcr
    ...
    cmp     command, 0                      ; MOV := 0x0000 + 0x100 * arg1 + 0x0800 * arg2
    je      _mov
    cmp     command, 2                      ; OR := 0x0002 + 0x100 * arg1 + 0x0800 * arg2
    je      _or
    cmp     command, 4                      ; ADD := 0x0004 + 0x100 * arg1 + 0x0800 * arg2
    je      _add
    cmp     command, 5                      ; SUB := 0x0005 + 0x100 * arg1 + 0x0800 * arg2
    je      _sub
    cmp     command, 8                      ; XCHG := 0x0008 + 0x100 * arg1 + 0x0800 * arg2
    je      _xchg
    cmp     command, 6                      ; ADC := 0x0006 + 0x100 * arg1 + 0x0800 * arg2
    je      _adc
_sbb:                                       ; SBB := 0x0007 + 0x100 * arg1 + 0x0800 * arg2
    SET_CARRY
    sbb     arg1, arg2
    jmp     zc_flags
_or:
